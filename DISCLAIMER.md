...
- **mailregime's core is offline.** The rules engine (`getEmailRules`, adapters, enrichers, audit-record builder) runs entirely in your process, returns data structures, and opens no network connections. It does not call home, does not transmit telemetry. The **optional** storage helper (`mailregime/store/postgres`) — only loaded when you explicitly import it — opens a connection to a Postgres database **YOU configure with YOUR connection string** and writes consent receipts to **YOUR database**. mailregime does not run a service, does not host data, and never sends data anywhere except the destination you specify.
