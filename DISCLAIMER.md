...
> **Read this before you use, fork, modify, deploy, or rely on mailregime in any way.**

## How this software was built (read this first)

- **mailregime was written largely with the help of an AI coding assistant (Claude Code).** No claim is made that the code is bug-free, security-hardened, semantically correct, or fit for any purpose. AI-assisted code can contain subtle errors that look plausible but are wrong.
...
- **mailregime's core is offline.** The rules engine (`getEmailRules`, adapters, enrichers, audit-record builder) runs entirely in your process, returns data structures, and opens no network connections. It does not call home, does not transmit telemetry. The **optional** storage helper (`mailregime/store`) — only loaded when you explicitly import it — wraps an ORM client **YOU pass in** (Prisma, Drizzle, Kysely, TypeORM, or MongoDB via fumadb adapters). mailregime never opens its own connection; whatever your existing ORM client is configured to talk to is where consent receipts land. mailregime does not run a service, does not host data, and never sends data anywhere except the destination you specify.
...
1. **mailregime is not legal advice.** It is a developer convenience library that returns structured data based on inputs. It is not a substitute for a lawyer, a Data Protection Officer, a compliance officer, or any other professional.
...
4. **Use at your own risk.** If you use mailregime and your company gets fined, sued, audited, regulated, embarrassed, shamed, blocklisted, or otherwise damaged, that is **your** problem — not the maintainers'. You agreed to that the moment you installed the package, ran the code, read the docs, or otherwise touched the project.

5. **You must get your own legal advice.** Before relying on any output of mailregime for an actual business decision, retain qualified counsel licensed in every jurisdiction relevant to your use case and have them sign off on (a) your specific configuration, (b) your specific consent flows, (c) your privacy notices and disclosures, (d) your data-subject-rights procedures, (e) your record-keeping, and (f) anything else they want to look at. The maintainers will never do this for you and have no obligation or capacity to.
...
- You cannot rely on mailregime in a regulator's view as evidence that you exercised due care.
- You cannot point at mailregime in litigation as the reason your consent collection was lawful.
...
- That lawyer represents the mailregime project, the maintainers, or you.
...
If you fork or redistribute mailregime, you must keep this DISCLAIMER and the LICENSE intact. If you modify the data and republish, the original maintainers carry zero responsibility for your modifications, and you must make that clear to your downstream users.

## Jurisdiction

The LICENSE and this disclaimer do not commit any maintainer, author, or contributor to any particular jurisdiction or forum. Any dispute related to use of mailregime is subject to the disclaimer of liability and waiver in the LICENSE, which the user accepted by using the software.
